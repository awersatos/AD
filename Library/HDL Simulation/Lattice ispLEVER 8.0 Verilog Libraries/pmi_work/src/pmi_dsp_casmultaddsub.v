...
// --------------------------------------------------------------------
//
// Simulation Library File for PMI DSP-based Cascade-Multiply-AddSub
//
...
module pmi_dsp_casmultaddsub
 #(parameter pmi_dataa_width = 18,
  ...
  (input [(pmi_dataa_width-1):0]       A0, 
   ...
   output SignSUM,
   output [53:0]  SUM,
   output reg [17:0]  SROA,
   output reg [17:0]  SROB)/*synthesis syn_black_box*/;
...
reg [53:0] sum_sig_i_e, sum_reg1;
reg [17:0] SROA_t, SROA_reg;
wire [53:0] sum_sig;
...
initial
begin
   if (pmi_pipelined_mode == "on" && (pmi_output_reg == "off" || pmi_additional_pipeline == 0))
   begin
      $display ("Error! Fully-pipelined mode must have both output and pipeline registers on!");
      $stop;
   end
   if (pmi_cascade_match_reg == "on" && pmi_input_reg == "off")
   begin
      $display ("Error! If cascade match register is on, input register must also be on!");
      $stop;
   end
end

    assign SignB_new = SignB;
    ...
    begin
         if (ShiftA0 == 1'b1)
//             a0_sig = SRIA;
             a0_sig = SRIA[17:17-(pmi_dataa_width-1)];
    ...
    begin
         if (ShiftB0 == 1'b1)
//             b0_sig = SRIB;
             b0_sig = SRIB[17:17-(pmi_datab_width-1)];
    ...
    assign sum_sig_i = addnsub_p2 ? $signed(p0_sig_i_e + p1_sig_i_e) : $signed(p0_sig_i_e - p1_sig_i_e);

    always @(sum_sig_i or signeda_p2 or signedb_p2)
    begin
      sum_sig_i_e[(36-(pmi_dataa_width + pmi_datab_width)):0] = 0;
      sum_sig_i_e[53:(36-(pmi_dataa_width + pmi_datab_width))] = sum_sig_i;
//      sum_sig_i_e[36:(36-(pmi_dataa_width + pmi_datab_width))] = sum_sig_i;
//      if ((signeda_p2 || signedb_p2) == 1'b1)
//           sum_sig_i_e[53:37] = {17{sum_sig_i[(pmi_dataa_width + pmi_datab_width)]}};
//      else
//           sum_sig_i_e[53:37] = 0;
    end

    assign sum_sig = CIN + sum_sig_i_e;
    ...
    always @(a1_sig_p)
    begin
      SROA_t[(18 - pmi_dataa_width):0] = 0;
      SROA_t[17:(18 - pmi_dataa_width)] = a1_sig_p;
    end

    always @(b1_sig_p)
    begin
      SROB[(18 - pmi_datab_width):0] = 0;
      SROB[17:(18 - pmi_datab_width)] = b1_sig_p;
    end
    ...
    assign SignSUM = signeda_p2 | signedb_p2;
//pragma translate_on

// $Header: /devl/xcs/repo/env/Databases/CAEInterfaces/versclibs/data/virtex4/X_FIFO16.v,v 1.14 2007/06/06 22:12:04 wloo Exp $
///////////////////////////////////////////////////////////////////////////////
...
module X_FIFO16 (ALMOSTEMPTY, ALMOSTFULL, DO, DOP, EMPTY, FULL, RDCOUNT, RDERR, WRCOUNT, WRERR, DI, DIP, RDCLK, RDEN, RST, WRCLK, WREN
	...
	case (DATA_WIDTH)
	   ...
	   default :
		begin
		    $display("Attribute Syntax Error : The attribute DATA_WIDTH on X_FIFO16 instance %m is set to %d.  Legal values for this attribute are 4, 9, 18 or 36.", DATA_WIDTH);
		    $finish;
		end
	endcase
	...
	case (FIRST_WORD_FALL_THROUGH)
	    ...
	    default : begin
		$display("Attribute Syntax Error : The attribute FIRST_WORD_FALL_THROUGH on X_FIFO16 instance %m is set to %s.  Legal values for this attribute are TRUE or FALSE.", FIRST_WORD_FALL_THROUGH);
		$finish;
	      end
	endcase

	if ((fwft == 1'b0) && ((ALMOST_EMPTY_OFFSET < 5) || (ALMOST_EMPTY_OFFSET > addr_limit - 4)))
	    $display("Attribute Syntax Error : The attribute ALMOST_EMPTY_OFFSET on X_FIFO16 instance %m is set to %d.  Legal values for this attribute are %d to %d", ALMOST_EMPTY_OFFSET, 5, addr_limit - 4);
	if ((fwft == 1'b0) && ((ALMOST_FULL_OFFSET < 4) || (ALMOST_FULL_OFFSET > addr_limit - 5)))
	    $display("Attribute Syntax Error : The attribute ALMOST_FULL_OFFSET on X_FIFO16 instance %m is set to %d.  Legal values for this attribute are %d to %d", ALMOST_FULL_OFFSET, 4, addr_limit - 5);

	if ((fwft == 1'b1) && ((ALMOST_EMPTY_OFFSET < 6) || (ALMOST_EMPTY_OFFSET > addr_limit - 3)))
	    $display("Attribute Syntax Error : The attribute ALMOST_EMPTY_OFFSET on X_FIFO16 instance %m is set to %d.  Legal values for this attribute are %d to %d", ALMOST_EMPTY_OFFSET, 6, addr_limit - 3);
    ...
    always @(rst_in or rden_in or wren_in) begin
        if (rst_in ==1 && rden_in==1 )
            $display("Warning : At time %t,  RDEN on X_FIFO16 instance %m is high when RST is high. RDEN should be low during reset.", $stime);
        if (rst_in ==1 && wren_in ==1)
            $display("Warning : At time %t,  WREN on X_FIFO16 instance %m is high when RST is high. WREN should be low during reset.", $stime);
    end
    ...
    begin
       ...
       if (rst_tmp1 == 0 && rst_tmp2 == 1) begin
        if ((rst_rdckreg[2] & rst_rdckreg[1] & rst_rdckreg[0]) == 0) begin
            $display("Error : At time %t,  RST high on X_FIFO16 instance %m is short than three RDCLK clock cycles. RST high need be more that three RDCLK clock cycles.", $stime);
	    rst_rdclk_flag = 1;
	end

        if ((rst_wrckreg[2] & rst_wrckreg[1] & rst_wrckreg[0]) == 0) begin
            $display("Error : At time %t,  RST high on X_FIFO16 instance %m  is short than three WRCLK clock cycles. RST high need be more that three WRCLK clock cycles.", $stime);
	    rst_wrclk_flag = 1;
	end
...
//*** Timing Checks Start here
	...
	$recrem (negedge RST, posedge RDCLK, 0:0:0, 0:0:0, notifier);
	$recrem (negedge RST, posedge WRCLK, 0:0:0, 0:0:0, notifier);

...
// /___/   /\     Filename : X_SIM_CONFIG_S6_SERIAL.v
// \   \  /  \    Timestamp : 
//  \___\/\___\
//
// Revision:
...
//  buf buf_dout (DOUT, ds_out);
  ...
  initial begin
    if (DEVICE_ID == 32'h0) begin
      $display("Attribute Error : The attribute DEVICE_ID on  X_SIM_CONFIG_S6_SERIAL instance %m is not set.");
     end
   end
  ...
  always @( init_b_t or  prog_b_t)
    if (prog_b_t == 0)
       ...
       if (prog_b_t == 1)  begin
          ...
          if (m_in != 2'b11) begin
             mode_sample_flag <=  0;
             $display("Error: input M is %h. Only Slave Serial mode M=11 supported on X_SIM_CONFIG_S6_SERIAL instance %m.", m_in);
          end
          else
  ...
  always @(posedge init_b_t )
       if (prog_b_t != 1)  begin
            if ($time != 0 )
       $display("Error: PROGB is not high when INITB goes high on X_SIM_CONFIG_S6_SERIAL instance %m at time %t.", $time);
    end

  always @(m_in)
    if (mode_sample_flag == 1 && persist_en == 1)
       $display("Error : Mode pine M[2:0] changed after rising edge of INITB on X_SIM_CONFIG_S6_SERIAL instance %m at time %t.", $time);
  ...
  always @(posedge prog_b_in or negedge prog_b_in)
    if (prog_b_in == 0) 
       ...
       if (prog_pulse_low < cfg_Tprog )
        $display("Error: Low time of PROGB is less than required minimum Tprogram time %d on X_SIM_CONFIG_S6_SERIAL instance %m at time %t.", cfg_Tprog, $time);
    end
               ...
               6'b001110 : 
                          if (conti_data_cnt == 5'b00001) begin
                             ...
                             if (idcode_tmp[27:0] != DEVICE_ID[27:0]) begin
                                id_error_flag <= 1;
                                $display("Error :  written value to IDCODE register is %h which does not match DEVICE ID %h on X_SIM_CONFIG_S6_SERIAL instance %m at time %t.", idcode_tmp, DEVICE_ID, $time);
                             end 
                             else
                             id_error_flag <= 0;
                          end
...
//                $display("Warning :  only Type 1 Packet supported on X_SIM_CONFIG_S6_SERIAL instance %m at time %t.", $time);
                cmd_wr_flag <= 0;
                type2_flag <= 1;
                conti_data_flag <= 1;

// $Header: /devl/xcs/repo/env/Databases/CAEInterfaces/versclibs/data/simprims/X_RAMB16_S18_S36.v,v 1.10 2005/05/11 21:15:29 wloo Exp $
///////////////////////////////////////////////////////////////////////////////
...
// /___/   /\     Filename : X_RAMB16_S18_S36.v
// \   \  /  \    Timestamp : Thu Mar 10 16:43:59 PST 2005
//  \___\/\___\
//
// Revision:
...
module X_RAMB16_S18_S36 (DOA, DOB, DOPA, DOPB, ADDRA, ADDRB, CLKA, CLKB, DIA, DIB, DIPA, DIPB, ENA, ENB, SSRA, SSRB, WEA, WEB);
	...
	case (SIM_COLLISION_CHECK)
	    ...
	    "ALL" : ;
	    default : begin
		          $display("Attribute Syntax Error : The Attribute SIM_COLLISION_CHECK on X_RAMB16_S18_S36 instance %m is set to %s.  Legal values for this attribute are ALL, NONE, WARNING_ONLY or GENERATE_X_ONLY.", SIM_COLLISION_CHECK);
		          $finish;
	              end
...
//		6'b011010 : begin data_collision <= 2'b00; display_wa_rb; end
    ...
    begin
	if (display_flag)
        $display("Memory Collision Error on X_RAMB16_S18_S36:%m at simulation time %.3f ns\nA read was performed on address %h (hex) of Port A while a write was requested to the same address on Port B. The write will be successful however the read value on Port A is unknown until the next CLKA cycle.", $time/1000.0, addra_int);
    end
    endtask
...
//		6'b101010 : begin data_collision_b_a <= 2'b00; display_wa_rb; end
	...
	if (ssrb_int == 0 && output_flag) begin
	    dob_out[addra_int[0:0] * 16 +: 16] <= 16'bX;
	end
	data_collision[0] <= 0;
    end
    ...
    always @(posedge data_collision_a_b[0]) begin
	if (ssrb_int == 0 && output_flag) begin
	    dob_out[addra_reg[0:0] * 16 +: 16] <= 16'bX;
	end
	data_collision_a_b[0] <= 0;
    end
...
// x parity start
    ...
    always @(posedge data_collision[0]) begin
	if (ssrb_int == 0 && output_flag) begin
	    dopb_out[addra_int[0:0] +: 2] <= 2'bx;
	end
    end

    always @(posedge data_collision_a_b[0]) begin
	if (ssrb_int == 0 && output_flag) begin
	    dopb_out[addra_reg[0:0] +: 2] <= 2'bx;
	end
    end
...
// x parity end
	...
	if (ena_int == 1'b1) begin
		...
		if (wea_int == 1'b1) begin
			...
			doa_out <= mem[addra_int[9:1]][addra_int[0:0] * 16 +: 16];
			dopa_out <= memp[addra_int[9:1]][addra_int[0:0] * 2 +: 2];

...
module arriaii_latch(D, ENA, PRE, CLR, Q);
...
//ADD_CYCLONEIV	parameter sim_init_config_is_application = "false";
//ADD_CYCLONEIV	parameter sim_init_watchdog_enabled = "false";
//ADD_CYCLONEIV	parameter operation_mode = "active_serial_remote";
...
module  arriaii_oscillator 
    (
    oscena,
//ADD_CYCLONEIIILS    clkout1,
//ADD_CYCLONEIIILS    observableoutputport,
//ADD_CYCLONEIV    observableoutputport,
    clkout 
    );
...
//ADD_CYCLONEIIILS    output clkout1;
//ADD_CYCLONEIIILS    output observableoutputport;
//ADD_CYCLONEIV    output observableoutputport;
    output clkout;
...
`endif
   ram_type = 0;
    ...
    // REMARRIAII dataout_a = 'b0;
    // REMARRIAII if (mode_is_dp || mode_is_bdp) dataout_b = 'b0;
        dataout_a = 'b0;
...
begin
    if (active_write_clear_a & we_a_reg)
        mem_invalidate = 1'b1;
 else if (re_a_reg & active_a_core)
...
begin
   if (mode_is_bdp & active_write_clear_b & we_b_reg)
        mem_invalidate = 1'b1;
  else if ((mode_is_dp | mode_is_bdp) & re_b_reg & active_b_core)
    begin
        if (primary_port_is_b)
        begin
            read_data_latch = 'bx;

...
--
----------------------------------------------------------------------------
-- Filename:  mult_gen_v6_0_non_seq.vhd
--                                   
-- Description:  
...
----------------------------------------------------------------------------
-- Structure:
--    mult_gen_v6_0.vhd calls mult_gen_non_seq.vhd for non sequential cores.
--    mult_gen_v6_0.vhd calls mult_gen_seq.vhd for sequential cores.
--    mult_gen_v6_0_seq.vhd calls this model for it's internal multiplier.
----------------------------------------------------------------------------
...
use xilinxcorelib.mult_pkg_v6_0.all;

use XilinxCoreLib.mult_gen_v6_0_services.all;
...
architecture behavioral of mult_gen_v6_0_non_seq is
...
----------------------------------------------------------------------------
   ...
   begin
      ...
      if (c_mult_type > 1) and (c_has_loadb = 0) then
         ...
         if (c_mult_type > 2) and (bin'length = 1) then
            if (c_has_a_signed = 1) and (c_b_type = c_unsigned) then
              return non_ccm_width;
            else
              return non_ccm_width-1;
            end if;
         elsif (c_mult_type > 2) and (a_width = 1) then
            if (c_has_a_signed = 1) and (c_b_type = c_unsigned) then
              return non_ccm_width;
            else
              return non_ccm_width-1;
   ...
   begin
      ...
      if ((all0(la) and (c_has_a_signed = 0 or lsigna /= 'X')) and not(anyX(lb))) 
        or (all0(lb) and (not(anyX(la)) and (c_has_a_signed = 0 or lsigna /= 'X' or c_mult_type < 2))) then -- and c_mult_type < 2) then
         product := (others => '0');
...
----------------------------------------------------------------------------
   ...
   constant b_is_1             : boolean := actual_b_width = 1 and set_b_value(b_input'length-1) = '1' and c_mult_type = 2 and c_b_type = c_unsigned;
   ...
   signal b_i            : std_logic_vector((find_ccm_b_width(str_to_slv(c_b_value, c_b_width), c_b_width, c_mult_type, c_has_loadb)-1) downto 0)                                                                                                           := (others => '0');
   signal reg_b          : std_logic_vector((find_ccm_b_width(str_to_slv(c_b_value, c_b_width), c_b_width, c_mult_type, c_has_loadb)-1) downto 0)                                                                                                           := (others => '0');
   signal reg_reg_b      : std_logic_vector((find_ccm_b_width(str_to_slv(c_b_value, c_b_width), c_b_width, c_mult_type, c_has_loadb)-1) downto 0)                                                                                                           := (others => '0');
...
---------------------------------------------------------------------------- 
   ...
   begin  -- process generate_inputs
      ...
      a_i <= a;
      if (c_predelay = 0 and c_mult_type /= 2) then
        b_i <= b;
      elsif (c_predelay = 0 and c_mult_type = 2) then
        b_i <= b((find_ccm_b_width(str_to_slv(c_b_value, c_b_width), c_b_width, c_mult_type, c_has_loadb)-1) downto 0);
      elsif (c_predelay = 1) then
        b_i <= reg_b;
      else
        b_i <= reg_reg_b;
      end if;
      if (c_has_aclr = 1) and not(c_baat = c_a_width and c_latency = 0 and c_reg_a_b_inputs = 0 and c_has_q = 0 and c_mult_type < 3) then -- Change 1910d.
         aclr_i <= aclr;
      else
         aclr_i <= '0';
      end if;
      if (c_has_sclr = 1) and not(c_baat = c_a_width and c_latency = 0 and c_reg_a_b_inputs = 0 and c_has_q = 0 and c_mult_type < 3) then
--         and not(c_mult_type = 3 and loadb_count /= -1) then
         sclr_i <= sclr;
      else
         sclr_i <= '0';
      end if;
      ...
      if (c_has_loadb = 1 and c_mult_type > 2) then
         loadb_i_nop <= loadb;
      ...
      if (c_has_swapb = 1 and c_mult_type > 2) then
         if (c_predelay = 0) then
            swapb_i <= swapb;
         elsif (c_predelay = 1) then
            swapb_i <= reg_swapb;
   ...
   begin  -- process register_inputs
      if (aclr_i'event and aclr_i = '1') then
         ...
         if (sclr_i = '1' and (ce_i = '1' or c_sync_enable = 0)) then
            dina_q   <= (others => '0');
            if (c_mult_type /= 3 or loadb_count = -1) then
              dinb_q   <= (others => '0');
            end if;
            nd_q_p   <= '0';
            nd_q_c   <= '0';
   ...
   begin
     if (aclr_i = '1') then
       ...
       if sclr_i = '1' then
         ...
         --reg_b <= b;
         reg_b <= b((find_ccm_b_width(str_to_slv(c_b_value, c_b_width), c_b_width, c_mult_type, c_has_loadb)-1) downto 0);
         reg_reg_b <= reg_b;
         reg_swapb <= swapb;
   ...
   begin  -- process multiply
      if not(c_baat < c_a_width) then
         ...
         if (c_reg_a_b_inputs = 1) and (c_mult_type < 3) then
           if (c_has_nd = 1) and ((c_has_q = 1 and c_has_o = 0 and c_mult_type = 2) or c_latency > 0) then -- c_mult_type = 2 is Change 51101a.
             a_in   <= dina_q;
             b_in   <= dinb_q;
             a_sign <= asign_q;
         ...
         elsif (c_mult_type >= 3 and c_reg_a_b_inputs = 1) then
           if (c_has_nd = 1) then
             if aclr_i = '1' then
               a_in   <= (others => '0');
               a_sign <= '0';
               ...
               if sclr_i = '1' and (ce_i = '1' or c_sync_enable = 0) then
                 a_in   <= (others => '0');
                 a_sign <= '0';
               elsif (nd_i = '1' and ce_i = '1') then -- and (c_mult_type /= 3 or loadb_count = -1 or loadb = '1')) then
                 a_in   <= a_i;
                 a_sign <= asign_i;
               end if;
             end if;
           ...
           if (c_mult_type = 3 and c_latency = 0 and c_has_o = 1 and c_has_nd = 1) then
           ...
           elsif c_mult_type = 3 then
             b_in <= dinb_q; --b_input;
           else
             b_in <= dinb_q;
           end if;
         else -- c_reg_a_b_inputs = 0.
           if c_mult_type < 3 or c_latency = 0 then
             a_in   <= a_i;
             a_sign <= asign_i;
           else
             if aclr_i = '1' then
               ...
               if sclr_i = '1' and (ce_i = '1' or c_sync_enable = 0) then
                 a_in <= (others => '0');
                 a_sign <= '0';
               elsif ce_i = '1' and (nd_i = '1' or c_has_nd = 0) then
                 a_in <= a_i;
                 a_sign <= asign_i;
               end if;
             end if;
           end if;
           if (c_mult_type = 3 and c_has_nd = 1) then
             if ((clk_i'event and clk_i = '1') 
               or (c_mult_type = 3 and c_latency = 0 and nd_i'event and nd_i = '1')) then
			   --if (sclr_i = '1' and (ce_i = '1' or c_sync_enable = 0)) then
               --  b_in <= (others => '0');
               --elsif (load_done_i = '0' or (nd_i = '1' and (ce_i = '1' or c_has_ce = 0))) then
   ...
   begin
     if c_baat = c_a_width then
       if c_has_swapb = 0 and c_has_loadb = 1 and c_mult_type = 3 then -- Reloadable ccm (stop during reload).
         if c_reg_a_b_inputs = 0 then
           ready <= nd_i and load_done_i;
         else
           ready <= ready_rl;
         end if;
       elsif c_mult_type > 1 then -- ccm and continue_during_reload rccm.
         if c_reg_a_b_inputs = 0 then
           ready <= nd_i;
         else
           ready <= nd_q;
   ...
   begin  -- process register_output
    if (c_baat = c_a_width) then
      ...
      --elsif c_mult_type > 2 and c_latency > 0 and c_reg_a_b_inputs = 0 then
      if c_mult_type > 2 and c_latency > 0 and c_reg_a_b_inputs = 0 then
        ...
        if (c_has_q = 0) and c_latency = 0 then
          ...
          if aclr_i = '1' and (c_mult_type < 2 or c_mem_type = 0 or b_is_1) then
            sub_product(0) <= (others => '0');
          elsif clk_i'event and clk_i = '1' then
            if sclr_i = '1' and (ce_i = '1' or c_sync_enable = 0) and 
              not(c_mult_type = 3 and loadb_count /= -1) then
              sub_product(0) <= (others => '0');
            elsif (c_mult_type < 2 and c_latency = 0 and ce_i = '1' and c_has_q = 1) then
              sub_product(0) <= product_i;
            elsif ((nd_q = '1' or (c_mult_type = 3 and loadb_count /= -1)) 
              or c_has_swapb = 1 or c_has_nd = 0 
                or (c_mult_type < 2 and c_latency = 0 and c_has_q = 0)) and ce_i = '1' then 
              sub_product(0) <= product_i;
            end if;
          end if;
        end if;
        ...
        elsif (c_has_nd = 1 and c_mult_type < 2 and not(c_latency = 1 and c_has_o = 1)) then
          if aclr_i = '1' and (c_mult_type < 2 or c_mem_type = 0) then
            sub_product(0) <= (others => '0');
          elsif clk_i'event and clk = '1' then
            if sclr_i = '1' and (ce_i = '1' or c_sync_enable = 0) 
              and not(c_mult_type = 3 and loadb_count /= -1) then
          ...
          if c_latency = 0 then
            if aclr_i = '1' and (c_mult_type < 2 or c_mem_type = 0 or b_is_1) then
              ...
              if sclr_i = '1' and (ce_i = '1' or c_sync_enable = 0) 
                and not(c_mult_type = 3 and loadb_count /= -1) then
                sub_product(0) <= (others => '0');
              elsif (nd_i = '1' or (c_mult_type = 3 and loadb_count /= -1)) and ce_i = '1' then
                sub_product(0) <= product_i;
              elsif ce_i = '1' then
              end if;
            end if;
            ...
            if aclr_i = '1' and (c_mult_type < 2 or c_mem_type = 0) then
              ...
              if sclr_i = '1' and (ce_i = '1' or c_sync_enable = 0) 
                and not(c_mult_type = 3 and loadb_count /= -1) then
                sub_product(0) <= (others => '0');
              elsif ce_i = '1' and ((nd_i = '1' or (c_mult_type = 3 and loadb_count /= -1)) or swapb_i = '1') then
                sub_product(0) <= product_i;
              elsif ce_i = '1' then
              end if;
            end if;
      ...
      -- Clear it.
      if ((aclr_i = '1')) then
        if (c_mem_type = 2 and c_mult_type > 2 and (c_latency > 1 or (c_mult_type = 3 and c_mem_type = 2 and c_latency = 1))) then --and (c_pipeline = 0 or c_latency < 4)) then -- Change 1910b/d and 2610a.
        ...
        if (sclr_i = '1' and (ce_i = '1' or c_sync_enable = 0)) 
          and not(c_mult_type = 3 and loadb_count /= -1) then
          if (c_mem_type = 0 or c_mult_type < 2 or c_sync_enable = 1 or ce_i = '1') then
            if (c_mult_type > 2 and c_mem_type = 2 and c_has_loadb = 1 and c_has_swapb = 0 and c_latency > 1 and loadb_count /= -1) then
              for n in c_latency-1 to c_latency loop
                sub_product(n) <= (others => '0');
              end loop;
            elsif (c_mult_type /= 3 or loadb_count = -1) then
              for n in 0 to c_latency loop
                sub_product(n)   <= (others => '0');
              end loop;  -- n
            else
          ...
          if (c_reg_a_b_inputs = 1 or c_has_nd = 0 or (c_mult_type > 1 or (c_mult_type < 2 and c_latency = 1 and c_has_o = 1)) or c_pipeline = 0) and c_latency > 0  then -- Temp change 2410a.
            for n in 1 to c_latency loop
   ...
   begin  -- process register_output
      if (c_baat = c_a_width) then
        if (aclr_i = 'X' or sclr_i = 'X' or ce_i = 'X') then -- or clk_i = 'X') then
          ...
          o_i   <= product_i;
          if (c_mem_type = 0 or c_mult_type < 2) then
            if (aclr_i = '1') then
              q_i <= (others => '0');
            else
              q_i   <= sub_product(0);
          ...
          if (c_mem_type = 2 and c_mult_type > 2 and c_latency > 2 and (c_pipeline = 0 or c_latency > 3)) then
            if aclr_i = '1' then
              o_i <= (others => '0');
              q_i <= (others => '0');
            else
          ...
          elsif (c_mem_type = 2 and c_mult_type > 2 and (c_latency = 2 or (c_pipeline = 1 and c_latency > 1))) then
            o_i   <= sub_product(c_latency - 1);
            if aclr_i = '1' then
              q_i <= (others => '0');
          ...
          if (c_reg_a_b_inputs = 1 or c_has_nd = 0 or c_mult_type > 1 or c_pipeline = 0 or c_latency < 1) 
            or (c_pipeline = 1 and c_has_nd = 1 and c_mult_type < 2 and c_latency = 1 and c_reg_a_b_inputs = 0 and c_has_q = 0) then
            rdy_i <= sub_rdy(c_latency);
          else
            rdy_i <= sub_rdy(c_latency - 1);
          end if;
        end if;
        if (aclr_i = 'X' or sclr_i = 'X' or ce_i = 'X' or clk_i = 'X') then
          rdy_q <= 'X';
          rfd_q <= 'X';
        elsif aclr_i = '1' then
          rdy_q <= '0';
       ...
       if c_has_aclr /= 0 and aclr_i = '1' then
         ...
         if (loadb_i = 'X') then
           ...
           if loadb_i = '1' then
             ...
             if (c_has_o = 1 and c_mult_type = 3 and c_has_swapb = 0 and loadb_count = 2
  ...
  begin
   ...
   begin
     if (c_mult_type < 2) then
       b_input <= b;
     elsif (c_mult_type < 3) or (not_loaded = 1 and loadb_i = '0') then
     else
       ...
       if (loadb_count = -1 or c_has_swapb = 1) then
         ...
         elsif(bank_sel = '1' and c_mult_type > 2 and not_loaded = 0 and loadb_count < 0) then -- Change 2910a.
             b_input <= bconst1;
         end if;
       else
       -- Loading.

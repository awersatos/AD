-- $Header: /devl/xcs/repo/env/Databases/CAEInterfaces/unimacro/data/devlib/vhdl/BRAM_SINGLE_MACRO.vhd,v 1.13 2009/08/27 18:34:44 vandanad Exp $
-------------------------------------------------------------------------------
  ...
  begin
    if (DEVICE = "VIRTEX5" or DEVICE = "VIRTEX6" or DEVICE = "SPARTAN6") then
      case wr_width is
        ...
        when 37 to 72 => if (func_bram_size /= "36Kb" and (DEVICE = "VIRTEX5" or DEVICE = "VIRTEX6") ) then
        ...
        when others => if(DEVICE = "VIRTEX5" or DEVICE = "VIRTEX6") then
                       write( Message, STRING'("Illegal value of Attribute WRITE_WIDTH : ") );
                       write ( Message, WRITE_WIDTH);
...
adr_vir : if (DEVICE = "VIRTEX5" or DEVICE = "VIRTEX6") generate
  ...
  di1_v5: if (DEVICE = "VIRTEX5" or DEVICE = "VIRTEX6") generate
    digena1 : if (write_p = TRUE  and ((BRAM_SIZE = "18Kb" and WRITE_WIDTH <= 36) or (BRAM_SIZE = "36Kb" and WRITE_WIDTH <= 72) ) ) generate
      begin 
        dip_pattern(dip_width-1 downto 0) <= DI(fin_wr_width-1 downto di_width) ;
        di_pattern (di_width-1 downto 0) <= DI(di_width-1 downto 0);
      end generate digena1;
  end generate di1_v5;
 -- begin s10
 di1_st : if (DEVICE = "SPARTAN6") generate
    digena1 : if (write_p = TRUE  and ((BRAM_SIZE = "9Kb" and WRITE_WIDTH <= 36) or (BRAM_SIZE = "18Kb" and WRITE_WIDTH <= 72) ) ) generate
      begin 
        dip_pattern(dip_width-1 downto 0) <= DI(fin_wr_width-1 downto di_width) ;
        di_pattern (di_width-1 downto 0) <= DI(di_width-1 downto 0);
      end generate digena1;
  ...
  di2_v5: if (DEVICE = "VIRTEX5" or DEVICE = "VIRTEX6") generate
    digena2 : if (write_p = FALSE and ((BRAM_SIZE = "18Kb" and WRITE_WIDTH <= 36) or (BRAM_SIZE = "36Kb" and WRITE_WIDTH <= 72) ) ) generate
      begin 
        di_pattern(fin_wr_width-1 downto 0) <= DI(fin_wr_width-1 downto 0);
      end generate digena2;
  end generate di2_v5;
 -- begin s11
 di2_st : if (DEVICE = "SPARTAN6") generate
    digen2 : if (write_p = FALSE and ((BRAM_SIZE = "9Kb" and WRITE_WIDTH <= 36) or (BRAM_SIZE = "18Kb" and WRITE_WIDTH <= 72) ) ) generate
    begin 
       di_pattern(fin_wr_width-1 downto 0) <= DI(fin_wr_width-1 downto 0);
    end generate digen2;
  end generate di2_st; -- end s11

  do1_v5: if (DEVICE = "VIRTEX5" or DEVICE = "VIRTEX6") generate
    dogena1 : if (read_p = TRUE and ((BRAM_SIZE = "18Kb" and READ_WIDTH <= 36) or (BRAM_SIZE = "36Kb" and READ_WIDTH <= 72) ) ) generate
      begin
        DO <= (dop_pattern(dop_width-1 downto 0) & do_pattern(do_width-1 downto 0));
      end generate dogena1;
  end generate do1_v5;
 -- begin s12
 do1_st : if (DEVICE = "SPARTAN6") generate
    dogena1 : if (read_p = TRUE and ((BRAM_SIZE = "9Kb" and READ_WIDTH <= 36) or (BRAM_SIZE = "18Kb" and READ_WIDTH <= 72) ) ) generate
      begin
        DO <= (dop_pattern(dop_width-1 downto 0) & do_pattern(do_width-1 downto 0));
     end generate dogena1;
  end generate do1_st; -- end s12

  do2_v5: if (DEVICE = "VIRTEX5" or DEVICE = "VIRTEX6") generate
    dogena2 : if (read_p = FALSE and ((BRAM_SIZE = "18Kb" and READ_WIDTH <= 36) or (BRAM_SIZE = "36Kb" and READ_WIDTH <= 72) ) ) generate
      begin
        DO <= do_pattern(fin_rd_width-1 downto 0);
      end generate dogena2;
  end generate do2_v5;
  -- begin s13
  do2_st : if (DEVICE = "SPARTAN6") generate
    dogena2 : if (read_p = FALSE and ((BRAM_SIZE = "9Kb" and READ_WIDTH <= 36) or (BRAM_SIZE = "18Kb" and READ_WIDTH <= 72) ) ) generate
      begin
        DO <= do_pattern(fin_rd_width-1 downto 0);
     end generate dogena2;
  end generate do2_st; -- begin s13

  we1 : if (DEVICE = "VIRTEX5" or DEVICE = "VIRTEX6") generate
 ...
 -- end s14
 r1 : rstram_pattern <= RST when (DO_REG = 0) else '0'; 

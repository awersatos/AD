-- $Header: /devl/xcs/repo/env/Databases/CAEInterfaces/vhdsclibs/data/simprims/rainier/VITAL/X_AFIFO36_INTERNAL.vhd,v 1.7 2010/01/14 19:38:16 fphillip Exp $
-------------------------------------------------------------------------------
...
-- /___/   /\     Filename : X_AFIFO36_INTERNAL.vhd
...
entity X_AFIFO36_INTERNAL is
  ...
  attribute VITAL_LEVEL0 of
     X_AFIFO36_INTERNAL : entity is true;

end X_AFIFO36_INTERNAL;

-- architecture body                    --

architecture X_AFIFO36_INTERNAL_V of X_AFIFO36_INTERNAL is

  attribute VITAL_LEVEL0 of
    X_AFIFO36_INTERNAL_V : architecture is true;
  ...
  --- Internal Signal Declarations
    ...
    signal update_from_write_prcs_sync : std_ulogic := '0';
    signal update_from_read_prcs_sync  : std_ulogic := '0';
  ...
  begin
     if (first_time) then
       ...
       if (EN_SYN = FALSE) then
         ...
         if (fwft_var = '0') then
           ...
           if ((rd_offset_int < 5) or (rd_offset_int > addr_limit_var - 5)) then
             ...
             write( Message, STRING'("ALMOST_EMPTY_OFFSET on X_AFIFO36_INTERNAL is set to ") );
           ...
           if ((wr_offset_int < 4) or (wr_offset_int > addr_limit_var - 5)) then
             write( Message, STRING'("Attribute Syntax Error : ") );
             write( Message, STRING'("The attribute ") );
             write( Message, STRING'("ALMOST_FULL_OFFSET on X_AFIFO36_INTERNAL is set to ") );
       ...
       if(fwft_var = '1' and EN_SYN = TRUE) then
          assert false
          report "DRC Error : First word fall through is not supported in synchronous mode on X_AFIFO36_INTERNAL."
          severity failure;
       end if;
       ...
       if(EN_SYN = FALSE and DO_REG = 0) then
          assert false
          report "DRC Error : DO_REG = 0 is invalid when EN_SYN is set to FALSE on X_AFIFO36_INTERNAL."
          severity failure;
       end if;
       ...
       if ((EN_ECC_WRITE = TRUE or EN_ECC_READ = TRUE) and DATA_WIDTH /= 72) then
          assert false
            report "DRC Error : The attribute DATA_WIDTH must be set to 72 when X_AFIFO36_INTERNAL is configured in the ECC mode."
          severity failure;
       end if;
...
--####################################################################
  ...
  begin
     if(RST_dly = '1') then
       if(RDEN_dly = '1') then
          assert false
          report "Warning : RDEN on X_AFIFO36_INTERNAL  is high when RST is high. RDEN should be low during reset."
          severity Warning;
       end if;

       if(WREN_dly = '1') then
          assert false
          report "Warning : WREN on X_AFIFO36_INTERNAL  is high when RST is high. WREN should be low during reset."
          severity Warning;
       end if;
     end if;
    ...
    rst_wrclk_flag <= '0';
       ...
       if(falling_edge(RST_dly)) then
         if((rst_rdckreg(2) ='0') or (rst_rdckreg(1) ='0') or (rst_rdckreg(0) ='0')) then  
             assert false
             report "Error : RST signal on X_AFIFO36_INTERNAL stays high for less than three RDCLK clock cycles. RST has to stay high for more than three RDCLK clock cycles"
             severity Error;
             rst_rdclk_flag <= '1';
         end if;
         ...
         if((rst_wrckreg(2) ='0') or (rst_wrckreg(1) ='0') or (rst_wrckreg(0) ='0')) then  
             assert false
             report "Error : RST signal on X_AFIFO36_INTERNAL stays high for less than three WRCLK clock cycles. RST has to stay high for more than three WRCLK clock cycles"
             severity Error;
             rst_wrclk_flag <= '1';
         end if;
  ...
  begin
     if((GSR_dly = '1') or (RST_dly = '1')) then
       ...
       if(rising_edge(RDCLK_dly)) then
	    ...
	    if (rd_offset_int <= 4 * (period_rdclk_var / period_wrclk)) then  -- in period

              write( Message, STRING'("DRC Error : ") );
              write( Message, STRING'("The attribute ") );
              write( Message, STRING'("ALMOST_EMPTY_OFFSET on X_AFIFO36_INTERNAL is set to ") );
              write( Message, rd_offset_int);
              write( Message, STRING'(". It must be set to a value greater than (4 * WRCLK frequency / RDCLK frequency) when AFIFO36_INTERNAL is configured in FIRST_WORD_FALL_THROUGH mode.") );
              ASSERT FALSE REPORT Message.ALL SEVERITY Failure;
              DEALLOCATE (Message);

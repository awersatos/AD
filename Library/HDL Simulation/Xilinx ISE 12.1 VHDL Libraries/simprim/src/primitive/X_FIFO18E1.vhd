-- $Header: /devl/xcs/repo/env/Databases/CAEInterfaces/vhdsclibs/data/simprims/blanc/VITAL/X_FIFO18E1.vhd,v 1.20 2010/01/14 19:35:25 fphillip Exp $
-------------------------------------------------------------------------------
...
-- WARNING !!!: The following X_FF18_INTERNAL_VHDL entity is not an user primitive. 
...
entity X_FF18_INTERNAL_VHDL is
...
end X_FF18_INTERNAL_VHDL;

-- architecture body                    --

architecture X_FF18_INTERNAL_VHDL_V of X_FF18_INTERNAL_VHDL is
  ...
  begin
     if (first_time) then
            ...
            when others =>
                    GenericValueCheckMessage
                      ( HeaderMsg            => " Attribute Syntax Error ",
                        GenericName          => " EN_SYN ",
                        EntityName           => "X_FF18_INTERNAL_VHDL",
                        GenericValue         => EN_SYN,
                        Unit                 => "",
       ...
       if (EN_SYN = FALSE) then
         ...
         if (fwft_var = '0') then
           ...
           if ((rd_offset_int < 5) or (rd_offset_int > addr_limit_var - 5)) then
             ...
             write( Message, STRING'("ALMOST_EMPTY_OFFSET on X_FF18_INTERNAL_VHDL is set to ") );
           ...
           if ((wr_offset_int < 4) or (wr_offset_int > addr_limit_var - 5)) then
             write( Message, STRING'("Attribute Syntax Error : ") );
             write( Message, STRING'("The attribute ") );
             write( Message, STRING'("ALMOST_FULL_OFFSET on X_FF18_INTERNAL_VHDL is set to ") );
       ...
       if(fwft_var = '1' and EN_SYN = TRUE) then
          assert false
          report "DRC Error : First word fall through is not supported in synchronous mode on X_FF18_INTERNAL_VHDL."
          severity failure;
       end if;
       ...
       if(EN_SYN = FALSE and DO_REG = 0) then
          assert false
          report "DRC Error : DO_REG = 0 is invalid when EN_SYN is set to FALSE on X_FF18_INTERNAL_VHDL."
          severity failure;
       end if;
       ...
       if ((EN_ECC_WRITE = TRUE or EN_ECC_READ = TRUE) and DATA_WIDTH /= 72) then
          assert false
            report "DRC Error : The attribute DATA_WIDTH must be set to 72 when X_FF18_INTERNAL_VHDL is configured in the ECC mode."
          severity failure;
       end if;
...
--####################################################################
  ...
  begin
     if(RST_dly = '1') then
       if(RDEN_dly = '1') then
          assert false
          report "Warning : RDEN on X_FF18_INTERNAL_VHDL  is high when RST is high. RDEN should be low during reset."
          severity Warning;
       end if;

       if(WREN_dly = '1') then
          assert false
          report "Warning : WREN on X_FF18_INTERNAL_VHDL  is high when RST is high. WREN should be low during reset."
          severity Warning;
       end if;
     end if;
    ...
    rst_wrclk_flag <= '0';
       ...
       if(falling_edge(RST_dly)) then
         if((rst_rdckreg(2) ='0') or (rst_rdckreg(1) ='0') or (rst_rdckreg(0) ='0')) then  
             assert false
             report "Error : RST signal on X_FF18_INTERNAL_VHDL stays high for less than three RDCLK clock cycles. RST has to stay high for more than three RDCLK clock cycles"
             severity Error;
             rst_rdclk_flag <= '1';
         end if;
         ...
         if((rst_wrckreg(2) ='0') or (rst_wrckreg(1) ='0') or (rst_wrckreg(0) ='0')) then  
             assert false
             report "Error : RST signal on X_FF18_INTERNAL_VHDL stays high for less than three WRCLK clock cycles. RST has to stay high for more than three WRCLK clock cycles"
             severity Error;
             rst_wrclk_flag <= '1';
         end if;
      ...
      if(rising_edge(WRCLK_dly)) then
        ...
        if (not(EN_ECC_WRITE = TRUE or EN_ECC_READ = TRUE)) then
          ...
          if (injectsbiterr_dly = '1') then
            assert false
              report "DRC Warning : INJECTSBITERR is not supported when neither EN_ECC_WRITE nor EN_ECCREAD = TRUE on X_FF18_INTERNAL_VHDL instance."
              severity Warning;
          end if;

          if (injectdbiterr_dly = '1') then
            assert false
            report "DRC Warning : INJECTDBITERR is not supported when neither EN_ECC_WRITE nor EN_ECCREAD = TRUE on X_FF18_INTERNAL_VHDL instance."
            severity Warning;
          end if;
...
end X_FF18_INTERNAL_VHDL_V;


-- end of X_FF18_INTERNAL_VHDL - Note: Not an user primitive
...
architecture X_FIFO18E1_V of X_FIFO18E1 is
  ...
  signal do_zd : std_logic_vector(31 downto 0) :=  (others => '0');
  signal dop_zd : std_logic_vector(3 downto 0) :=  (others => '0');
  ...
  signal rdcount_zd : std_logic_vector(11 downto 0) :=  (others => '0');
  signal wrcount_zd : std_logic_vector(11 downto 0) :=  (others => '0');

  signal DIP_ipd : std_logic_vector(3 downto 0);
  signal DI_ipd : std_logic_vector(31 downto 0);
  ...
  signal DIP_dly : std_logic_vector(3 downto 0);
  signal DI_dly : std_logic_vector(31 downto 0);
...
X_FIFO18E1_inst : X_FF18_INTERNAL_VHDL
  generic map (
...
--  Setup/Hold Check Violations (all input pins)
     ...
     VitalSetupHoldCheck
       (
         ...
         RefTransition  => 'R',
         HeaderMsg      => InstancePath & "/X_FIFO18E1",
         Xon            => Xon,
         MsgOn          => MsgOn,
         MsgSeverity    => Warning
       );
     ...
     Violation_rdclk <= Tviol_RST_RDCLK_negedge or
...
--  Output Pin glitch declaration
     ...
     variable  DO_viol     : std_logic_vector(31 downto 0);
     variable  DOP_viol    : std_logic_vector(3 downto 0);
     ...
     variable  RDCOUNT_viol    : std_logic_vector(11 downto 0);
     variable  WRCOUNT_viol    : std_logic_vector(11 downto 0);

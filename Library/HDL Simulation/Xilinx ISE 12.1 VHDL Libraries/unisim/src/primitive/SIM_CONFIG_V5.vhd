...
-- /___/   /\     Filename : sim_config_v5.vhd
-- \   \  /  \    Timestamp : 
--  \___\/\___\
--
-- Revision:
...
architecture SIM_CONFIG_V5_V of SIM_CONFIG_V5 is
   ...
   rdwr_b_in <= RDWRB;
        ...
        begin
          if (DEVICE_ID = X"00000000") then
             assert FALSE report "Attribute Syntax Error : The attribute DEVICE_ID on  SIM_CONFIG_V5 is not set." severity error;
          end if;
          wait;
        end process;
...
--   wait on rst_en, init_rst, prog_b_in, iprog_b;
      ...
      if (mode_sample_flag = '0') then
        if(prog_b_t = '1') then
           ...
           if (m_in /= "110") then
              ...
              Write ( Message, string'(" . Only Slave SelectMAP mode M=110 supported on SIM_CONFIG_V5."));
              assert false report Message.all severity error;
              DEALLOCATE (Message);
           end if;
        elsif (NOW > 0 ps) then
            assert false report "Error: PROGB is not high when INITB goes high on SIM_CONFIG_V5." severity error;
       end if;
    end if;
   end if;
  end process;
  ...
  process (m_in) begin
    if (mode_sample_flag = '1' and persist_en = '1') then
       assert false report "Error : Mode pine M[2:0] changed after rising edge of INITB on SIM_CONFIG_V5." severity error;
    end if;
  end process;
           ...
           if (bus_en = '1'  and  rdwr_b_in  =  '0')  then
                 ...
                 if (buswid_flag_init  =  '0') then
                     ...
                     if (tmp_byte  =  X"11") then
                         ...
                         buswid_flag_init <= '0';
             assert false report 
                         "Error : BUS Width Auto Dection did not find 0x11 or 0x22 or 0x44 on D(7:0) followed 0xBB on SIM_CONFIG_V5."
              severity error;
                     end if;
                 end if;
            end if;
    ...
    begin
      if (rst_intl = '0'  or  abort_flag_rst = '1'  or  csi_b_in  =  '1') then
        ...
        if (abort_dis  =  '0'  and  csi_b_in  =  '0') then
             if (NOW  /=  0 ps) then
               abort_flag_wr <= '1';
               assert false report " Warning : RDWRB changes when CS_B low, which causes Configuration abort on SIM_CONFIG_V5."
               severity warning;
             end if;
        else
           abort_flag_wr <= '0';

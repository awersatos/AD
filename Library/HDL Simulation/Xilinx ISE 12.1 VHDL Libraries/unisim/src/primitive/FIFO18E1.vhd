-- $Header: /devl/xcs/repo/env/Databases/CAEInterfaces/vhdsclibs/data/unisims/blanc/VITAL/FIFO18E1.vhd,v 1.14 2009/11/19 22:13:05 wloo Exp $
-------------------------------------------------------------------------------
  ...
  begin
     if (first_time) then
       ...
       if (EN_SYN = FALSE) then
         ...
         if (fwft_var = '0') then
           ...
           if ((rd_offset_int < 5) or (rd_offset_int > addr_limit_var - 5)) then
             ...
             write( Message, STRING'("ALMOST_EMPTY_OFFSET on FF18_INTERNAL_VHDL is set to ") );
           ...
           if ((wr_offset_int < 4) or (wr_offset_int > addr_limit_var - 5)) then
             write( Message, STRING'("Attribute Syntax Error : ") );
             write( Message, STRING'("The attribute ") );
             write( Message, STRING'("ALMOST_FULL_OFFSET on FF18_INTERNAL_VHDL is set to ") );
       ...
       if(fwft_var = '1' and EN_SYN = TRUE) then
          assert false
          report "DRC Error : First word fall through is not supported in synchronous mode on FF18_INTERNAL_VHDL."
          severity failure;
       end if;
       ...
       if(EN_SYN = FALSE and DO_REG = 0) then
          assert false
          report "DRC Error : DO_REG = 0 is invalid when EN_SYN is set to FALSE on FF18_INTERNAL_VHDL."
          severity failure;
       end if;
       ...
       if ((EN_ECC_WRITE = TRUE or EN_ECC_READ = TRUE) and DATA_WIDTH /= 72) then
          assert false
            report "DRC Error : The attribute DATA_WIDTH must be set to 72 when FF18_INTERNAL_VHDL is configured in the ECC mode."
          severity failure;
       end if;
...
--####################################################################
  ...
  begin
     if(RST_dly = '1') then
       if(RDEN_dly = '1') then
          assert false
          report "Warning : RDEN on FF18_INTERNAL_VHDL  is high when RST is high. RDEN should be low during reset."
          severity Warning;
       end if;

       if(WREN_dly = '1') then
          assert false
          report "Warning : WREN on FF18_INTERNAL_VHDL  is high when RST is high. WREN should be low during reset."
          severity Warning;
       end if;
     end if;
    ...
    rst_wrclk_flag <= '0';
       ...
       if(falling_edge(RST_dly)) then
         if((rst_rdckreg(2) ='0') or (rst_rdckreg(1) ='0') or (rst_rdckreg(0) ='0')) then  
             assert false
             report "Error : RST signal on FF18_INTERNAL_VHDL stays high for less than three RDCLK clock cycles. RST has to stay high for more than three RDCLK clock cycles"
             severity Error;
             rst_rdclk_flag <= '1';
         end if;
         ...
         if((rst_wrckreg(2) ='0') or (rst_wrckreg(1) ='0') or (rst_wrckreg(0) ='0')) then  
             assert false
             report "Error : RST signal on FF18_INTERNAL_VHDL stays high for less than three WRCLK clock cycles. RST has to stay high for more than three WRCLK clock cycles"
             severity Error;
             rst_wrclk_flag <= '1';
         end if;
      ...
      if(rising_edge(WRCLK_dly)) then
        ...
        if (not(EN_ECC_WRITE = TRUE or EN_ECC_READ = TRUE)) then
          ...
          if (injectsbiterr_dly = '1') then
            assert false
              report "DRC Warning : INJECTSBITERR is not supported when neither EN_ECC_WRITE nor EN_ECCREAD = TRUE on FF18_INTERNAL_VHDL instance."
              severity Warning;
          end if;

          if (injectdbiterr_dly = '1') then
            assert false
            report "DRC Warning : INJECTDBITERR is not supported when neither EN_ECC_WRITE nor EN_ECCREAD = TRUE on FF18_INTERNAL_VHDL instance."
            severity Warning;
          end if;

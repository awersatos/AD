...
static ssize_t terminal_to_serial_write(devio_t *dev);
static ssize_t terminal_to_serial_read(devio_t *dev);
...
extern void terminal_to_serial_init(void)
{
    ...
    terminal_to_serial_cfg_instance_t *cfg;
    const char *name;
...
static ssize_t terminal_to_serial_write(devio_t *dev)
{
    ...
    dev->iotx->cnt = terminal_write(terminal_dev->terminal, (const char *)dev->iotx->buf, dev->iotx->len);
...
static ssize_t terminal_to_serial_read(devio_t *dev)
{
    ...
    dev->iorx->cnt = terminal_read(terminal_dev->terminal, (char *)dev->iorx->buf, dev->iorx->len);

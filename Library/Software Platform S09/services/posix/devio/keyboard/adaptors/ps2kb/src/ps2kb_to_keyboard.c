...
static ssize_t  ps2kb_to_keyboard_write ( devio_t *dev ) { return ENOTSUP; }
static ssize_t  ps2kb_to_keyboard_read  ( devio_t *dev );
...
extern void ps2kb_to_keyboard_init( void )
{
    ...
    ps2kb_to_keyboard_cfg_instance_t * cfg;
...
static int ps2kb_to_keyboard_ctl(devio_t *dev, int cmd, void *buf, size_t size, int *res)
{
    ...
    case DEVCTL_KEYBOARD_LEDS:
        ps2kb_setleds(kbdev->ps2kb,leds->leds);
        break;
    default:
        break;
    }
...
inline bool ps2kb_to_keyboard_update_shift_state(ps2kb_to_keyboard_t *kbd, uint8_t val)
{
    ...
    case KEY_CAPSLOCK:
        ...
        ps2kb_setleds(kbd->ps2kb, kbd->shift_state & 0x07);
        break;
    case KEY_NUMLOCK:
...
inline uint8_t ps2kb_to_keyboard_handle_shift_state(ps2kb_to_keyboard_t *kbd, uint8_t val)
{
    if ( val >= 'a' && val <= 'z' )
    {
        if (kbd->shift_state & (SHIFT_LEFT | SHIFT_RIGHT | SHIFT_CAPS))
        {
            val -= ('a' - 'A');
        }
    }

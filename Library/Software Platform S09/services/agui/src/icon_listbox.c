...
void icon_listbox_draw(obj_t *obj)
{
    ...
    while ( item && (( y + string_height) <= y2 ))
    {
        ...
        if (invalidated || item->invalidated)
        {
            ...
            agui_cut_string_indicated(agui, str, item->text, sizeof(str)-1, string.font, string.fontstyle, w - 3 * string.x - 2 - BITMAP_SIZE);
            ...
            agui_draw_string(agui, x1 + BITMAP_SIZE + string.x, y, x2, y + string_height, &string, obj->enabled);
    ...
    while ( item && (( y + line_height ) <= y2 ))
    {
        ...
        if (invalidated || item->invalidated)
        {
            ...
            agui_cut_string_indicated(agui, str, item->text, sizeof(str)-1, string.font, string.fontstyle, w - 2 * string.x - 2 - BITMAP_SIZE - AGUI_CORNER_RADIUS);
            ...
            if (top)
            {
                ...
                agui_draw_string(agui, x1 + BITMAP_SIZE + AGUI_CORNER_RADIUS, y + 1, x2, y + 1 + string_height, &string, obj->enabled);
            }
            else
            {
                if (item->icon)
                ...
                agui_draw_string(agui, x1 + BITMAP_SIZE + AGUI_CORNER_RADIUS, y, x2, y + string_height, &string, obj->enabled);
            }
...
int icon_listbox_delete( icon_listbox_t *listbox, int index )
{
    ...
    if (icon_listbox_count(listbox) == 0)
    {
        return -1;
    }

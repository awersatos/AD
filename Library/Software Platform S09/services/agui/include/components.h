...
typedef struct
{
    ...
    bool     pressed; /**< true if the button is pressed, set by mouse-input handler of the button */
...
typedef struct
{
    ...
    int              select;             /**< last selected entry, for iternal use */
 ...
 *
 * This structure describes a listbox with icons.
 * The listbox generates callback events for mouse-down and mouse-up.
 *
 * @see
...
typedef struct
{
    ...
    bool     pressed; /**< checkbox is pressed (mouse-down above checkbox) */
    relief_t relief;  /**< relief: flat, raised, lowered etc. */

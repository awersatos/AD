...
static void usbhost_mouse_synccomplete(usbhost_urb_t *urb)
{
...
static bool usbhost_mouse_devicetest(void *devicedrv, usbhost_device_t *usbdevice)
{
    ...
    usbhost_initurb(urb);
    usbhost_buildasyncinterrupturb(usbdevice, &usbdevice->config->intf[0]->curIntf->endpoint[0], mousedata, MOUSEDATA_SIZE, &usbhost_mouse_synccomplete, urb);
...
usbhost_mouse_t *usbhost_mouse_open(int id)
{
    ...
    usbhost_mouse_cfg_instance_t *restrict drv_cfg = &usbhost_mouse_instance_table[id];
    usbhost_t *usbhost;

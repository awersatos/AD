...
void usbhost_buildcontrolendpoint(usbhost_host_endpoint_t *ep, uint8_t direction)
{
    ...
    ep->desc.bDescriptorType   = 0x05;
    ep->desc.bEndpointAddress  = 0x00 | direction;
    ep->desc.bmAttributes      = 0x00;
...
//    dev->epIN[0]  = &dev->ep0IN;
//    dev->epOUT[0] = &dev->ep0OUT;
...
void usbhost_endpointinit(usbhost_endpointdescriptor_t *epDesc, uint32_t enabled, usbhost_host_endpoint_t *ep)
{
    memcpy(&ep->desc, epDesc, sizeof(usbhost_endpointdescriptor_t));
    ep->enabled = enabled;
    ep->hcPriv = NULL;
// EA removed, never used    INIT_LIST_HEAD(&ep->urbList);
}
...
usbhost_interface_t *usbhost_interfacealloc(uint32_t count)
{
    return (usbhost_interface_t *)calloc(sizeof(usbhost_interface_t), count);
}


void usbhost_interfacefree(usbhost_interface_t *intf)
{
    free(intf);
}

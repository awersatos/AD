...
#define DEFAULT_CTRL_REG 0x1380 // divider on, color-indexed, disabled
...
vga_1to8bpp_t *vga_1to8bpp_open(unsigned int id)
{
    ...
    drv_vga_1to8bpp_cfg_instance_t * restrict drv_cfg = &drv_vga_1to8bpp_instance_table[id];
    // get configuration peripheral below driver (created by plugin system)
    per_vga32_cfg_instance_t * restrict per_cfg = &per_vga32_instance_table[drv_cfg->per_vga32];
...
{
    drv_vga_1to8bpp_cfg_instance_t * restrict drv_cfg = &drv_vga_1to8bpp_instance_table[0];

...
m25px0_t * m25px0_open( unsigned id )
{
    ...
    if ( drv->spi == NULL )
    {
        const drv_m25px0_cfg_instance_t * restrict drv_cfg = &drv_m25px0_instance_table[id];
...
#if __BIG_ENDIAN__
        spi_set_endianess( drv->spi, true );                                // Switch endianess to native format
#elif __LITTLE_ENDIAN__
        spi_set_endianess( drv->spi, false );                               // Switch endianess to native format
#endif

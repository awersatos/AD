...
# error S29 driver does not support FLASH_SHIFT_ADDR_LINES != 0
#endif
...
int s29_device_init( s29_t * restrict  s29, uint32_t baseaddr )
{
    ...
    volatile uint16_t *ebri_count_l;
    volatile uint16_t *ebri_count_h;
    volatile uint16_t *ebri_size_l;
    volatile uint16_t *ebri_size_h;
    size_t size;
    uint16_t blocks;
    size_t blksize;
    uint16_t regions;
    ...
    // low/high addressed depend on endianness
    ebri_count_l = flash + FLASH_ADDR_CFI_EBRI_COUNT_L;
    ebri_count_h = flash + FLASH_ADDR_CFI_EBRI_COUNT_H;
    ebri_size_h = flash + FLASH_ADDR_CFI_EBRI_SIZE_H;
    ebri_size_l = flash + FLASH_ADDR_CFI_EBRI_SIZE_L;

    // Go through the list of erase block regions and get the individual sizes
    for ( int index = 0; index < regions; index++ )
    {
        blocks = ( *ebri_count_l | ( *ebri_count_h << 8 ) ) + 1;
        blksize = ( size_t ) 256 *( *ebri_size_l | ( *ebri_size_h << 8 ) );
        size += blksize * blocks;
        ebri_count_l += 4;
        ebri_count_h += 4;
        ebri_size_l += 4;
...
/**
 * @brief
 *      Initialize the S29 parallel flash device
 *
...
{
    s29_t * s29;
    const drv_s29_cfg_instance_t * cfg;
    ...
    if (s29->baseaddr == 0)
    {
        ...
        {
            s29->baseaddr = 0; // reset
        }
        S29_MUTEX_UNLOCK( s29 );
    }
    return s29->baseaddr != 0 ? s29 : NULL;
}
...
extern int s29_program_block( s29_t * restrict  s29, size_t offset, void *memptr, size_t bytes )
{
    ...
    for ( index = 0; index < words; index += bufsize, num_words -= bufsize )
    {
        ...
        flash[FLASH_ADDR_UNLOCK1] = FLASH_CMD_PRE1;
        flash[FLASH_ADDR_UNLOCK2] = FLASH_CMD_PRE2;
        flash[offset] = FLASH_CMD_WBL;
        flash[offset] = n - 1;
        for ( int j = 0; j < n; j++ )
        {
            flash[index + j + offset] = *mem++;
        }

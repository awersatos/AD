...
terminal_t *terminal_open(int id)
{
    ...
    const drv_terminal_cfg_instance_t * restrict drv_cfg = &drv_terminal_instance_table[id];
    // get configuration peripheral below driver (created by plugin system)
    const per_terminal_cfg_instance_t * restrict per_cfg = &per_terminal_instance_table[drv_cfg->per_terminal];

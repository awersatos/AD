...
irrc_t *irrc_open(int id)
{
    ...
    if (drv->baseaddr == 0)
    {
        ...
        const drv_irrc_cfg_instance_t * restrict drv_cfg = &drv_irrc_instance_table[id];
        // get configuration peripheral below driver (created by plugin system)
        const per_irrc_cfg_instance_t * restrict per_cfg = &per_irrc_instance_table[drv_cfg->per_irrc];

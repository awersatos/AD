 ...
 *
 * @return  Driver pointer if successful, NULL otherwise.
 */
pwm8_t * pwm8_open(int id)
{
    ...
    if (drv->base_addr == 0)
    {
        ...
        const drv_pwm8_cfg_instance_t * restrict drv_cfg = &drv_pwm8_instance_table[id];
        // get configuration of peripheral below driver (created by plugin system)
        const per_pwm8_cfg_instance_t * restrict per_cfg = &per_pwm8_instance_table[drv_cfg->per_pwm8];

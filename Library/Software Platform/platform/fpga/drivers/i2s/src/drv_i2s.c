...
i2s_t * i2s_open(int id)
{
    ...
    if (drv->base_addr == 0)
    {
        ...
        const drv_i2s_cfg_instance_t * restrict drv_cfg = &drv_i2s_instance_table[id];
        // get configuration of peripheral below driver (created by plugin system)
        const per_i2s_cfg_instance_t * restrict per_cfg = &per_i2s_instance_table[drv_cfg->per_i2s];
        ...
        if (drv->revision == 0)
        {
            ...
            {
                // legacy core driver only runs with software buffers
                drv->base_addr = 0;
                return NULL;
            }
        }

...
#define TFT_ENABLED     (VGA32_TFT_LITTLEENDIAN | VGA32_TFT_CKDE | VGA32_TFT_VOP | VGA32_TFT_HOP | VGA32_TFT_VSE)
#define TFT_DISABLED    (VGA32_TFT_LITTLEENDIAN | VGA32_TFT_CKDE | VGA32_TFT_VOP | VGA32_TFT_HOP)
#else
...
vga_tft_t * vga_tft_open(unsigned int id)
{
    ...
    const drv_vga_tft_cfg_instance_t * restrict drv_cfg = &drv_vga_tft_instance_table[id];
    // get configuration peripheral below driver (created by plugin system)
    const per_vga32_tft_cfg_instance_t * restrict per_cfg = &per_vga32_tft_instance_table[drv_cfg->per_vga32_tft];

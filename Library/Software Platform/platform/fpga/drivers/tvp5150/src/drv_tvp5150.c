...
// phase1
// - start
// - write slave address for writing
// - ack
...
// phase2
// - start
// - write slave address for reading
// - ack
// - read data
// - nack
//
inline int tvp5150_getreg( i2cm_t * restrict i2cm, uint8_t busaddress, uint8_t reg )
{
         ...
         && i2cm_putchar( i2cm, false, reg ) == 0 )
    {
        ...
        // phase 2. read data
        if ( i2cm_putchar( i2cm, true, busaddress | 0x01 ) == 0 )
        {
...
{
    tvp5150_t *tvp5150;
    const drv_tvp5150_cfg_instance_t *cfg;
    int status;
#if ( DRV_TVP5150_USE_MUTEX != 0 )
    ...
    if ( tvp5150->i2cm == NULL )
    {
        ...
        if ( tvp5150->i2cm != NULL )
        {
            ...
            status = i2cm_get_bus( tvp5150->i2cm );
    ...
    return tvp5150->i2cm != NULL ? tvp5150 : NULL;
}

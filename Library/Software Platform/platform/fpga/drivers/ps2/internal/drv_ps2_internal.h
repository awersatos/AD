...
struct ps2_s
{
    ...
    size_t          tail;
    void (*interrupt_notify)(ps2_t * restrict ps2_dev, void *user_data);
    void *          user_data;
#if __POSIX_KERNEL__
    int             wait_mode;
...
inline int ps2_busywait(ps2_t * restrict ps2_dev)
{
...
// read one byte
inline int ps2_get_byte(ps2_t * restrict ps2_dev)
{
    int retval = -1;
...
inline int ps2_timed_get_byte(ps2_t * restrict ps2_dev)
{
    int retval = -1;
    uint64_t start_mark;
...
inline int ps2_wait_for_byte(ps2_t * restrict ps2_dev, uint8_t val)
{
    int retval = -1;
    uint64_t start_mark;
...
// write one byte
inline int ps2_put_byte(ps2_t * restrict ps2_dev, uint8_t val)
{
    int retval = -1;

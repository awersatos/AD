...
// Fixed reponse timeout for slave reset in i2cm_open()
// Must be long enough for slow slave devices to respond.
#define I2CM_OPEN_TIMEOUT_MS    100
...
# endif
        ...
        clock_t timeout_delay = I2CM_OPEN_TIMEOUT_MS * CLOCKS_PER_SEC / 1000;
        for( int i = 0; i < 9; i++ )
        {
            clock_t endtick;

            I2CM_CTRL( drv->baseaddress ) = I2CM_CTRL_ENABLE  | I2CM_CTRL_IEN | I2CM_CTRL_STO;  // Make stop condition
            endtick = clock() + timeout_delay;
            while (1)
            {
                if ((I2CM_STAT( drv->baseaddress ) & I2CM_STAT_INTREQ) != 0)
                {
                    break;
                }
                threadwait(drv);   // Wait for operation to finish
                if (clock() > endtick) // timeout: no slave device?
                {
                    drv->baseaddress = 0; // baseaddress is used as init flag
                    return NULL;
                }
            }

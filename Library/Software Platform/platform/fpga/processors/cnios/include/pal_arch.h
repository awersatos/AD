...
inline void   nios_posix_interrupts_internal_enable ()
{
    ...
    if (status & NIOS2_STATUS_INTERRUPT_ENABLE)
    {
        ...
        nios_set_enabled_interrupts( mask | nios_posix_interrupts_savemask | (1<<PAL_ARCH_NIOS_TIMER_INTERRUPT_NUMBER));
        nios_enable_interrupts();
    }
    else
    {
...
inline void   nios_posix_interrupts_internal_disable ()
{
    ...
    if (status & NIOS2_STATUS_INTERRUPT_ENABLE)
    {
        nios_disable_interrupts();
        mask = nios_get_enabled_interrupts();
        nios_posix_interrupts_savemask = mask & (posix_interrupts_bitmask | (1<<PAL_ARCH_NIOS_TIMER_INTERRUPT_NUMBER));
        mask &= ~(posix_interrupts_bitmask | (1<<PAL_ARCH_NIOS_TIMER_INTERRUPT_NUMBER));
        nios_set_enabled_interrupts( mask );
        nios_enable_interrupts();
    }
    else

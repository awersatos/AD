...
void uath_register_driver(void)
{
    int i;
    for(i=0;i<(sizeof(uath_devs)/sizeof(struct uath_type));i++){
        usbd_Register_device(UHUB_UNK_PORT, 1, 1, uath_devs[i].dev.ud_vendor, uath_devs[i].dev.ud_product, UHUBCF_RELEASE_DEFAULT, &uath_ca, &uath_cd);
    }
}

void bsd_drv_uath_init(void)
{
#if __POSIX_KERNEL__
    posix_mainhook_install(uath_register_driver, 5);
#endif
}
...
int
uath_cmd_read(struct uath_softc *sc, uint32_t code, const void *idata, int ilen, void *odata, int flags)
{
    flags |= UATH_CMD_FLAG_READ;
...
uath_reset(struct uath_softc *sc)
{
    ...
    s = splusb();
    error = uath_cmd_write(sc, UATH_CMD_SETUP, &setup, sizeof setup, UATH_CMD_FLAG_ASYNC);
...
uath_init(struct ifnet *ifp)
{
    ...
    error = uath_cmd_read(sc, UATH_CMD_07, NULL, 0, &val, UATH_CMD_FLAG_MAGIC);
    if (error != 0) {

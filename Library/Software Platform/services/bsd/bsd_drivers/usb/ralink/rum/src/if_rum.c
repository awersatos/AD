...
void        rum_setup_tx_desc   (struct rum_softc *, struct rum_tx_desc *, uint32_t, uint16_t, int, int);
int         rum_tx_data         (struct rum_softc *, struct mbuf *, struct ieee80211_node *);
...
void        rum_select_band     (struct rum_softc *, struct ieee80211_channel *);
...
#endif
void        rum_newassoc        (struct ieee80211com *, struct ieee80211_node *, int);
...
void rum_register_driver(void)
{
    int i;
    for(i=0;i<(sizeof(rum_devs)/sizeof(struct usb_devno));i++){
        usbd_Register_device(UHUB_UNK_PORT, 1, 1, rum_devs[i].ud_vendor, rum_devs[i].ud_product, UHUBCF_RELEASE_DEFAULT, &rum_ca, &rum_cd);
    }
}

void bsd_drv_rum_init(void)
{
#if __POSIX_KERNEL__
    posix_mainhook_install(rum_register_driver, 5);
#endif
}
...
#ifdef RUM_DYNAMIC_FIRMWARE_LOAD
void
rum_attachhook(void *xsc)
{
    ...
    if ((error = loadfirmware(name, &ucode, &size)) != 0) {
        DPRINTF(("%s: failed loadfirmware of file %s (error %d)\n",
            sc->sc_dev.dv_xname, name, error));
        return;
    }

    if (rum_load_microcode(sc, ucode, size) != 0) {
        DPRINTF(("%s: could not load 8051 microcode\n",
            sc->sc_dev.dv_xname));
    }
...
{
    struct rum_softc *sc = xsc;
    if(rum_load_microcode(sc, rt2573, sizeof(rt2573))!=0){
        DPRINTF(("%s: could not load 8051 microcode\n", sc->sc_dev.dv_xname));
    }
}
#endif
...
rum_attach(struct device *parent, struct device *self, void *aux)
{
    ...
    if (usbd_set_config_no(sc->sc_udev, RT2573_CONFIG_NO, 0) != 0) {
        DPRINTF(("%s: could not set configuration no\n",
            sc->sc_dev.dv_xname));
        return;
    }
    ...
    error = usbd_device2interface_handle(sc->sc_udev, RT2573_IFACE_INDEX,
        &sc->sc_iface);
    if (error != 0) {
        DPRINTF(("%s: could not get interface handle\n",
            sc->sc_dev.dv_xname));
        return;
    }
    ...
    for (i = 0; i < id->bNumEndpoints; i++) {
        ...
        if (ed == NULL) {
            DPRINTF(("%s: no endpoint descriptor for iface %d\n",
    ...
    if (ntries == 1000) {
        DPRINTF(("%s: timeout waiting for chip to settle\n", sc->sc_dev.dv_xname));
        return;
    }
    ...
    DPRINTF(("\n\n%s: MAC/BBP RT%04x (rev 0x%05x), RF %s, address %s\n",
...
rum_alloc_tx_list(struct rum_softc *sc)
{
    ...
    for (i = 0; i < RUM_TX_LIST_COUNT; i++) {
        ...
        if (data->xfer == NULL) {
            DPRINTF(("%s: could not allocate tx xfer\n",
                sc->sc_dev.dv_xname));
            error = ENOMEM;
            goto fail;
        }
        ...
        if (data->buf == NULL) {
            DPRINTF(("%s: could not allocate tx buffer\n",
                sc->sc_dev.dv_xname));
            error = ENOMEM;
            goto fail;
        }
...
rum_alloc_rx_list(struct rum_softc *sc)
{
    ...
    for (i = 0; i < RUM_RX_LIST_COUNT; i++) {
        ...
        data->xfer = usbd_alloc_xfer(sc->sc_udev);
        if (data->xfer == NULL) {
            DPRINTF(("%s: could not allocate rx xfer\n",
                sc->sc_dev.dv_xname));
            error = ENOMEM;
            goto fail;
        }
        if (usbd_alloc_buffer(data->xfer, MCLBYTES) == NULL) {
            DPRINTF(("%s: could not allocate rx buffer\n",
                sc->sc_dev.dv_xname));
            error = ENOMEM;
            goto fail;
        }
        ...
        if (!(data->m->m_flags & M_EXT)) {
            DPRINTF(("%s: could not allocate rx mbuf cluster(1)\n",
                sc->sc_dev.dv_xname));
            error = ENOMEM;
            goto fail;
        }
...
rum_task(void *arg)
{
    ...
    case IEEE80211_S_AUTH:
        DPRINTF(("IEEE80211_S_AUTH\n"));
        rum_set_chan(sc, ic->ic_bss->ni_chan);
        break;

    case IEEE80211_S_ASSOC:
        DPRINTF(("IEEE80211_S_ASSOC\n"));
        rum_set_chan(sc, ic->ic_bss->ni_chan);
        break;

    case IEEE80211_S_RUN:
        DPRINTF(("IEEE80211_S_RUN\n"));
        rum_set_chan(sc, ic->ic_bss->ni_chan);
...
rum_txeof(usbd_xfer_handle xfer, usbd_private_handle priv, usbd_status status)
{
    ...
    if (status != USBD_NORMAL_COMPLETION) {
        ...
        DPRINTF(("%s: could not transmit buffer: %s\n",
            sc->sc_dev.dv_xname, (unsigned char *)usbd_errstr(status)));
...
rum_rxeof(usbd_xfer_handle xfer, usbd_private_handle priv, usbd_status status)
{
    ...
    if (!(mnew->m_flags & M_EXT)) {
        DPRINTF(("%s: could not allocate rx mbuf cluster(2)\n", sc->sc_dev.dv_xname));
        m_freem(mnew);
        ifp->if_ierrors++;
        goto skip;
    }
...
skip:   /* setup a new transfer */
    usbd_setup_xfer(xfer, sc->sc_rx_pipeh, data, data->buf, MCLBYTES, USBD_SHORT_XFER_OK, USBD_NO_TIMEOUT, rum_rxeof);
...
rum_tx_data(struct rum_softc *sc, struct mbuf *m0, struct ieee80211_node *ni)
{
    ...
    if (needrts || needcts) {
        ...
        if (mprot == NULL) {
            DPRINTF(("%s: could not allocate protection frame\n",
                sc->sc_dev.dv_xname));
            m_freem(m0);
            return ENOBUFS;
        }
    ...
    /* mbuf is no longer needed */
    m_freem(m0);
    usbd_setup_xfer(data->xfer, sc->sc_tx_pipeh, data, data->buf, xferlen, USBD_FORCE_SHORT_XFER | USBD_NO_COPY, RUM_TX_TIMEOUT, rum_txeof);
...
rum_eeprom_read(struct rum_softc *sc, uint16_t addr, void *buf, int len)
{
    ...
    if (error != 0) {
        DPRINTF(("%s: could not read EEPROM: %s\n",
            sc->sc_dev.dv_xname, (unsigned char *)usbd_errstr(error)));
    }
}
...
rum_read_multi(struct rum_softc *sc, uint16_t reg, void *buf, int len)
{
    ...
    error = usbd_do_request(sc->sc_udev, &req, buf);
    if (error != 0) {
        //DPRINTF(("%s: could not multi read MAC register: %s\n", sc->sc_dev.dv_xname, usbd_errstr(error)));
        DPRINTF(("Could not multi read MAC register\n"));
    }
}
...
rum_write_multi(struct rum_softc *sc, uint16_t reg, void *buf, size_t len)
{
    ...
    error = usbd_do_request(sc->sc_udev, &req, buf);
    if (error != 0) {
        DPRINTF(("%s: could not multi write MAC register\n",sc->sc_dev.dv_xname));
    }
}
...
rum_bbp_write(struct rum_softc *sc, uint8_t reg, uint8_t val)
{
    ...
    if (ntries == 5) {
        DPRINTF(("%s: could not write to BBP\n", sc->sc_dev.dv_xname));
        return;
    }
...
rum_rf_write(struct rum_softc *sc, uint8_t reg, uint32_t val)
{
    ...
    if (ntries == 5) {
        DPRINTF(("%s: could not write to RF\n", sc->sc_dev.dv_xname));
        return;
    }
...
#define N(a)    (sizeof (a) / sizeof ((a)[0]))
    ...
    if (ntries == 100) {
        DPRINTF(("%s: timeout waiting for BBP\n",
            sc->sc_dev.dv_xname));
        return EIO;
    }
...
#define N(a)    (sizeof (a) / sizeof ((a)[0]))
    ...
    if (ntries == 1000) {
        DPRINTF(("%s: timeout waiting for BBP/RF to wakeup\n",
            sc->sc_dev.dv_xname));
        goto fail;
    }
    ...
    if (sc->amrr_xfer == NULL) {
        DPRINTF(("%s: could not allocate AMRR xfer\n",
            sc->sc_dev.dv_xname));
        goto fail;
    }
    ...
    error = usbd_open_pipe(sc->sc_iface, sc->sc_tx_no, USBD_EXCLUSIVE_USE,
        &sc->sc_tx_pipeh);
    if (error != 0) {
        DPRINTF(("%s: could not open Tx pipe: %s\n",
            sc->sc_dev.dv_xname, (unsigned char *)usbd_errstr(error)));
        goto fail;
    }
    error = usbd_open_pipe(sc->sc_iface, sc->sc_rx_no, USBD_EXCLUSIVE_USE,
        &sc->sc_rx_pipeh);
    if (error != 0) {
        DPRINTF(("%s: could not open Rx pipe: %s\n",
            sc->sc_dev.dv_xname, (unsigned char *)usbd_errstr(error)));
        goto fail;
    }
    ...
    if (error != 0) {
        DPRINTF(("%s: could not allocate Tx list\n",
            sc->sc_dev.dv_xname));
        goto fail;
    }
    error = rum_alloc_rx_list(sc);
    if (error != 0) {
        DPRINTF(("%s: could not allocate Rx list\n",
            sc->sc_dev.dv_xname));
        goto fail;
    }
    ...
    for (i = 0; i < RUM_RX_LIST_COUNT; i++) {
        ...
        error = usbd_transfer(data->xfer);
        if (error != 0 && error != USBD_IN_PROGRESS) {
            DPRINTF(("%s: could not queue Rx transfer\n",
                sc->sc_dev.dv_xname));
            goto fail;
        }
    }
...
rum_load_microcode(struct rum_softc *sc, const u_char *ucode, size_t size)
{
    ...
    if (error != 0) {
        DPRINTF(("%s: could not run firmware: %s\n",
            sc->sc_dev.dv_xname, (unsigned char *)usbd_errstr(error)));
    }
    return error;
}
...
rum_prepare_beacon(struct rum_softc *sc)
{
    ...
    if (m0 == NULL) {
        DPRINTF(("%s: could not allocate beacon frame\n",
            sc->sc_dev.dv_xname));
        return ENOBUFS;
    }
...
rum_amrr_timeout(void *arg)
{
    ...
    usbd_setup_default_xfer(sc->amrr_xfer, sc->sc_udev, sc, USBD_DEFAULT_TIMEOUT, &req, sc->sta, sizeof sc->sta, 0, rum_amrr_update);
    ...
    usbd_status status)
{
    ...
    if (status != USBD_NORMAL_COMPLETION) {
        DPRINTF(("%s: could not retrieve Tx statistics - cancelling "

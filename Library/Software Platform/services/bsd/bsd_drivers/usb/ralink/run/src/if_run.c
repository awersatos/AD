 ...
 */
 ...
 // http://www.openbsd.org/cgi-bin/man.cgi?query=run&apropos=0&sektion=0&manpath=OpenBSD+Current&arch=i386&format=html
...
#ifdef RUN_DEBUG
#define RUN_DPRINTF(x)  do { if (run_debug) printf x; } while (0)
#define RUN_DPRINTFN(n, x)  do { if (run_debug >= (n)) printf x; } while (0)
int run_debug = 15;
#else
#define RUN_DPRINTF(x)
#define RUN_DPRINTFN(n, x)
#endif
...
void run_register_driver(void)
{
    int i;
    for(i=0;i<(sizeof(run_devs)/sizeof(struct usb_devno));i++){
        usbd_Register_device(UHUB_UNK_PORT, 1, 1, run_devs[i].ud_vendor, run_devs[i].ud_product, UHUBCF_RELEASE_DEFAULT, &run_ca, &run_cd);
    }
}

void bsd_drv_run_init(void)
{
#if __POSIX_KERNEL__
    posix_mainhook_install(run_register_driver, 5);
#endif
}
...
run_attach(struct device *parent, struct device *self, void *aux)
{
    ...
    if (usbd_set_config_no(sc->sc_udev, 1, 0) != 0) {
        RUN_DPRINTF(("%s: could not set configuration no\n", sc->sc_dev.dv_xname));
        return;
    }
    ...
    if (error != 0) {
        RUN_DPRINTF(("%s: could not get interface handle\n", sc->sc_dev.dv_xname));
        return;
    }
    ...
    if (nrx < 1 || ntx < 4) {
        RUN_DPRINTF(("%s: missing endpoint\n", sc->sc_dev.dv_xname));
        return;
    }
    ...
    if (ntries == 100) {
        RUN_DPRINTF(("%s: timeout waiting for NIC to initialize\n", sc->sc_dev.dv_xname));
        return;
    }
    sc->mac_ver = ver >> 16;
    ...
    RUN_DPRINTF(("%s: MAC/BBP RT%04X (rev 0x%04X), RF %s (MIMO %dT%dR), "
...
run_load_microcode(struct run_softc *sc)
{
...
#ifdef RUN_DYNAMIC_FIRMWARE_LOAD
    if ((error = loadfirmware(fwname, &ucode, &size)) != 0) {
        RUN_DPRINTF(("%s: failed loadfirmware of file %s (error %d)\n", sc->sc_dev.dv_xname, fwname, error));
        return error;
    }
#endif

    if (size != 4096) {
        RUN_DPRINTF(("%s: invalid firmware size (should be 4KB)\n", sc->sc_dev.dv_xname));
        //free(ucode, M_DEVBUF);
        return EINVAL;
    }
    ...
    if (ntries == 1000) {
        RUN_DPRINTF(("%s: timeout waiting for MCU to initialize\n", sc->sc_dev.dv_xname));
        return ETIMEDOUT;
    }
    RUN_DPRINTF(("microcode successfully loaded after %d tries\n", ntries));
    return 0;
}
...
run_read_eeprom(struct run_softc *sc)
{
    ...
    RUN_DPRINTF(("EEPROM rev=%d, FAE=%d\n", val & 0xff, val >> 8));
    ...
    RUN_DPRINTF(("EEPROM freq offset %d\n", sc->freq & 0xff));
    if ((val >> 8) != 0xff) {
    ...
    RUN_DPRINTF(("EEPROM LED mode=0x%02x, LEDs=0x%04x/0x%04x/0x%04x\n",
        sc->leds, sc->led[0], sc->led[1], sc->led[2]));

    /* read RF information */
    run_srom_read(sc, RT2860_EEPROM_ANTENNA, &val);
    if (val == 0xffff) {
        RUN_DPRINTF(("invalid EEPROM antenna info, using default\n"));
    ...
    RUN_DPRINTF(("EEPROM RF rev=0x%02x chains=%dT%dR\n",
        sc->rf_rev, sc->ntxchains, sc->nrxchains));

    run_srom_read(sc, RT2860_EEPROM_CONFIG, &val);
    RUN_DPRINTF(("EEPROM CFG 0x%04x\n", val));
    ...
    for (i = 0; i < 14; i++) {
        ...
        if (sc->txpow2[i] < 0 || sc->txpow2[i] > 31)
            sc->txpow2[i] = 5;
        RUN_DPRINTF(("chan %d: power1=%d, power2=%d\n",
    ...
    RUN_DPRINTF(("power compensation=%d (2GHz), %d (5GHz)\n",
    ...
    for (ridx = 0; ridx < 5; ridx++) {
        ...
        RUN_DPRINTF(("ridx %d: power 20MHz=0x%08x, 40MHz/2GHz=0x%08x, "
    ...
    if (sc->lna[2] == 0 || sc->lna[2] == 0xff) {
        RUN_DPRINTF(("invalid LNA for channel group %d\n", 2));
        sc->lna[2] = sc->lna[1];
    }
    ...
    for (ant = 0; ant < 3; ant++) {
        if (sc->rssi_2ghz[ant] < -10 || sc->rssi_2ghz[ant] > 10) {
            RUN_DPRINTF(("invalid RSSI%d offset: %d (2GHz)\n",
                ant + 1, sc->rssi_2ghz[ant]));
            sc->rssi_2ghz[ant] = 0;
        }
        if (sc->rssi_5ghz[ant] < -10 || sc->rssi_5ghz[ant] > 10) {
            RUN_DPRINTF(("invalid RSSI%d offset: %d (5GHz)\n",
...
run_node_alloc(struct ieee80211com *ic)
{
    ...
    return calloc(sizeof (struct run_node), 1);
}
...
run_newstate_cb(struct run_softc *sc, void *arg)
{
    ...
    case IEEE80211_S_SCAN:
        RUN_DPRINTF(("IEEE80211_S_SCAN\n"));
        run_set_chan(sc, ic->ic_bss->ni_chan);
        timeout_add_msec(&sc->scan_to, 200);
        break;

    case IEEE80211_S_AUTH:
        RUN_DPRINTF(("IEEE80211_S_AUTH\n"));
    case IEEE80211_S_ASSOC:
        RUN_DPRINTF(("IEEE80211_S_ASSOC\n"));
        run_set_chan(sc, ic->ic_bss->ni_chan);
        break;

    case IEEE80211_S_RUN:
        RUN_DPRINTF(("IEEE80211_S_RUN\n"));
        run_set_chan(sc, ic->ic_bss->ni_chan);
    ...
    struct ieee80211_key *k)
{
    ...
    if ((ic->ic_if.if_flags & (IFF_UP | IFF_RUNNING)) != (IFF_UP | IFF_RUNNING))
        return 0;
#endif
...
run_calibrate_cb(struct run_softc *sc, void *arg)
{
    ...
    RUN_DPRINTF(("retrycnt=%d txcnt=%d failcnt=%d\n",
...
run_newassoc(struct ieee80211com *ic, struct ieee80211_node *ni, int isnew)
{
    ...
    RUN_DPRINTF(("new assoc isnew=%d addr=%s\n",
    ...
    for (i = 0; i < rs->rs_nrates; i++) {
        ...
        RUN_DPRINTF(("rate=0x%02x ridx=%d ctl_ridx=%d\n",
...
run_rx_frame(struct run_softc *sc, uint8_t *buf, int dmalen)
{
    ...
    if (__predict_false(len > dmalen)) {
        RUN_DPRINTF(("bad RXWI length %u > %u\n", len, dmalen));
        return;
    }
...
run_rxeof(usbd_xfer_handle xfer, usbd_private_handle priv, usbd_status status)
{
    ...
    while (xferlen > 8) {
        ...
        if (__predict_false(dmalen == 0 || (dmalen & 3) != 0)) {
            RUN_DPRINTF(("bad DMA length %u\n", dmalen));
            break;
        }
        if (__predict_false(dmalen + 8 > xferlen)) {
            RUN_DPRINTF(("bad DMA length %u > %d\n",
                dmalen + 8, xferlen));
            break;
        }
...
run_tx(struct run_softc *sc, struct mbuf *m, struct ieee80211_node *ni)
{
    ...
    // BL[INFO] - Attempt to recover from send timeout
    if(ring->queued >= RUN_TX_RING_COUNT - 1){
        RUN_DPRINTF(("ring->queued: %i\n", ring->queued));
        RUN_DPRINTF(("))))))))))))))))))))))))))))) RESET RING QUEUE\n"));
        ring->queued = 1;
        sc->qfullmsk = 0;
        usbhost_abort_endpoint(ring->pipeh);
    }
...
run_init(struct ifnet *ifp)
{
    ...
    if ((error = run_load_microcode(sc)) != 0) {
        RUN_DPRINTF(("%s: could not load 8051 microcode\n", sc->sc_dev.dv_xname));
        goto fail;
    }
    ...
    if (ntries == 100) {
        RUN_DPRINTF(("%s: timeout waiting for DMA engine\n", sc->sc_dev.dv_xname));
        error = ETIMEDOUT;
        goto fail;
    }
    tmp &= 0xff0;
    ...
    if ((error = run_reset(sc)) != 0) {
        RUN_DPRINTF(("%s: could not reset chipset\n", sc->sc_dev.dv_xname));
        goto fail;
    }
    ...
    if ((error = run_bbp_init(sc)) != 0) {
        RUN_DPRINTF(("%s: could not initialize BBP\n", sc->sc_dev.dv_xname));
        goto fail;
    }

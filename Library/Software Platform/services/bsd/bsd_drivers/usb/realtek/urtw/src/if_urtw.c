...
void urtw_register_driver(void)
{
    int i;
    for(i=0;i<(sizeof(urtw_devs)/sizeof(struct urtw_type));i++){
        usbd_Register_device(UHUB_UNK_PORT, 1, 1, urtw_devs[i].dev.ud_vendor, urtw_devs[i].dev.ud_product, UHUBCF_RELEASE_DEFAULT, &urtw_ca, &urtw_cd);
    }
}

void bsd_drv_urtw_init(void)
{
#if __POSIX_KERNEL__
    posix_mainhook_install(urtw_register_driver, 5);
#endif
}
...
urtw_start(struct ifnet *ifp)
{
    ...
    for (;;) {
        ...
        if (m0 != NULL) {
            printf("sc->sc_tx_normal_queued: %i\n", sc->sc_tx_normal_queued);
...
urtw_task(void *arg)
{
    ...
    case IEEE80211_S_SCAN:
        printf("IEEE80211_S_SCAN\n");
        urtw_set_chan(sc, ic->ic_bss->ni_chan);
        //timeout_add(&sc->scan_to, bsdtick / 5);
        timeout_add_msec(&sc->scan_to, 200);
        break;

    case IEEE80211_S_AUTH:
        printf("IEEE80211_S_AUTH\n");
    case IEEE80211_S_ASSOC:
        printf("IEEE80211_S_ASSOC\n");
        urtw_set_chan(sc, ic->ic_bss->ni_chan);
        break;

...
/**
 * @brief    Draw string of at most n character on canvas
 *
 * Draw a string on the canvas. Draw at most @em count characters. If the string has less than @em count characters, the whole string is drawn.
 *
 * The string is drawn in the given font. Anti-aliased fonts are supported.
 * If no font is specified (font = NULL pointer), the default font is used.
 * The fontstyle is used to draw the string bold and/or italic independent of the used font.
 ...
 * @param color     Color as SVG-color or RGB-color.
 * @param fontstyle Fontstyle for the character, independent of the font
 * @param count     Maximum number of characters drawn
 *
 * @return Right location of last character
 */

int graphics_draw_nstring(canvas_t *canvas, int x, int y, const char *str, const font_t *font, color_t color, fontstyle_t fontstyle, int count)
{
    if (!font)
    {
        font = canvas->default_font;
...
/**
 * @brief    Get string width for at most n characters
 *
 * Calculate the width of the string when drawn with the given font. At most @em count characters are used in the calculation.
 * If the string has less than @em count characters, the width of the whole string is calculated.
 * If no font is specified (font = NULL pointer), the default font is used.
 *
 * @param srv       Service pointer as returned from graphics_open()
 * @param str       Pointer to string
 * @param font      Pointer to font
 * @param fontstyle Fontstyle for the string, independent of the font
 * @param count     Maximum number of characters used.
 *
 * @return Width of the string
 */

int graphics_get_nstringwidth(canvas_t *canvas,  const char *str, const font_t *font, fontstyle_t fontstyle, int count )
{
    int result = 0;
    uint8_t uc;
    int index;

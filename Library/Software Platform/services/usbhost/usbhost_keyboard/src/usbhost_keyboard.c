...
static void usbhost_keyboard_complete(usbhost_urb_t *urb)
{
...
static bool usbhost_keyboard_devicetest(void *devicedrv, usbhost_device_t *usbdevice)
{
...
#if DEBUG_USBHOST
    printf("HID Keyboard device connected\n");
#endif
    ...
    usbhost_buildinterrupturb(usbdevice, &usbdevice->config->intf[0].curIntf->endpoint[0], keyboarddata, KEYBOARDDATA_SIZE, &usbhost_keyboard_complete, NULL, urb);
...
usbhost_keyboard_t *usbhost_keyboard_open(int id)
{
    ...
    const usbhost_keyboard_cfg_instance_t *restrict drv_cfg = &usbhost_keyboard_instance_table[id];
    usbhost_t *usbhost;

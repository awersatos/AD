...
static ssize_t blockio_ide_write( devio_t *dev, ioreq_t *iotx )
{
    return EINVAL;
}
static ssize_t blockio_ide_read( devio_t *dev, ioreq_t *iorx )
{
    return EINVAL;
}
...
{
    const ide_to_blockio_cfg_instance_t *cfg;
    devio_t *dev;
    int ide_id;
    const char *name;

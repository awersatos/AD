...
static ssize_t uart8_to_serial_write(devio_t *dev, ioreq_t *iotx);
static ssize_t uart8_to_serial_read(devio_t *dev, ioreq_t *iorx);
...
extern void uart8_to_serial_init(void)
{
    ...
    const uart8_to_serial_cfg_instance_t *cfg;
    const char *name;
...
static ssize_t uart8_to_serial_write(devio_t *dev, ioreq_t *iotx)
{
    ...
    iotx->cnt = uart8_write(uart8_dev->uart8, (const char *)iotx->buf, iotx->len);
...
static ssize_t uart8_to_serial_read(devio_t *dev, ioreq_t *iorx)
{
    ...
    iorx->cnt = uart8_read(uart8_dev->uart8, (char *)iorx->buf, iorx->len);

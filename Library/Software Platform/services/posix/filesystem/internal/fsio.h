...
#include "posix_filesystem_mounting_cfg.h"

// Limits
#define MAX_OPEN_FILES          OPEN_MAX
#define MAX_OPEN_DIRS           OPEN_MAX
#define FD_TABLE_SIZE           MAX_OPEN_FILES + 3
#define MOUNT_TABLE_SIZE        2 + POSIX_FILESYSTEM_MOUNTING_MOUNT_MAX

// Keep in sync with number of supported file systems
#define MAX_FS                  POSIX_FILESYSTEM_MOUNTING_FS_MAX
...
struct fileops_s
{
    int (*f_read)( file_t *file, ioreq_t *iorx );
    int (*f_write)( file_t *fil, ioreq_t *iotx );
#if ( __POSIX_MAPPED_FILES != 0 )
...
// Internal access to rootmountpoints for use by rootfs
extern const char *fs_mountpoint( int index );

// Internal access to filedescriptor structure for use by lwip
extern file_t *fs_fd_file( int fd );

// Internal helper function to convert file descriptor pointer to file descriptor for use by lwip
extern int fs_fd( file_t *file );
...
extern int f_read_notsup( file_t *file, ioreq_t *iorx );
extern int f_write_notsup( file_t *file, ioreq_t *iotx );
#if ( __POSIX_MAPPED_FILES != 0 )

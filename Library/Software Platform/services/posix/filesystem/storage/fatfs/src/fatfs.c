...
// file ops
static int fatfs_read( file_t * restrict file, ioreq_t * restrict iorx );
static int fatfs_write( file_t * restrict file, ioreq_t * restrict iotx );
...
static int fatfs_read( file_t * restrict file, ioreq_t * restrict iorx )
{
    ...
    else
    {
        ...
        iorx->cnt = fat_read( fcb, iorx->buf, iorx->len );
...
static int fatfs_write( file_t * restrict file, ioreq_t * restrict iotx )
{
    ...
    else
    {
        FATFS_MUTEX_PREPARE();
        FATFS_MUTEX_LOCK( fcb->fs );
        iotx->cnt = fat_write( fcb, iotx->buf, iotx->len );
        FATFS_MUTEX_UNLOCK( fcb->fs );
    }

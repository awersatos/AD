...
// file ops
static int devfs_read( file_t *file, ioreq_t *iorx );
static int devfs_write( file_t *file, ioreq_t *iotx );
#if ( __POSIX_MAPPED_FILES != 0 )
...
static int devfs_read( file_t *file, ioreq_t *iorx )
{
    return devio_readwrite( file->data, iorx, false );
}

static int devfs_write( file_t *file, ioreq_t *iotx )
{
    return devio_readwrite( file->data, iotx, true );
}

...
/*
 * Initialize (clear) all fields, but does not init mutex and conditions
 */
static void posix_mqueue_init_fields(struct posix_mqueue_t *mqueue)
{
    mqueue->name[0]     = '\0';
    mqueue->maxmsg      = 0;
    mqueue->msgsize     = 0;
...
void    posix_mqueue_init(void)
{
    ...
    for (i = 0, mqueue = &_posix_mqueue_table[0]; i < MQ_OPEN_MAX; i++, mqueue++)
    {
        posix_mqueue_init_fields(mqueue);
                        ...
                        const struct timespec *timeout )
{
    ...
    if (msg_prio > MQ_PRIO_MAX || msg_ptr == NULL)
    {
        /* The value of msg_prio was outside the valid range or msg_ptr == NULL */
        retval = EINVAL;
    }
    ...
    else if (msg_len > mqueue->msgsize)
    {
    ...
    else
    {
        ...
        while (mqueue->count == mqueue->maxmsg && retval == 0)
        {
            ...
            else
            {
                ...
                if (timeout != NULL)
                {
                    ...
                    ticks = posix_timespec_to_ticks(&_posix_system_clock, timeout);
                }
                else
                {
#endif
                ...
                /* block until space in the queue for new messages */
                self->osmutex = 1;
                retval = __pthread_cond_timedwait(&mqueue->txcond, &mqueue->mutex, ticks);
        ...
        if (retval == 0)
        {
            ...
            {
                hdr = mqueue->freelist;
                mqueue->freelist = mqueue->freelist->next;
            }
            else
            {
                for (hdrp = &mqueue->freelist; !(*hdrp)->next->busy; hdrp = &(*hdrp)->next);
                hdr = (*hdrp)->next;
                (*hdrp)->next   = hdr->next;
            }
            ...
            {
                /* insert into uselist */
                for (hdrp = &mqueue->uselist; NULL != *hdrp; hdrp = &((*hdrp)->next))
                {
    ...
    else
    {
        ...
        while (mqueue->count == 0 && retval == 0)
        {
            ...
            else
            {
                ...
                self->osmutex = 1;
                retval = __pthread_cond_timedwait(&mqueue->rxcond, &mqueue->mutex, ticks);
        ...
        if (retval == 0)
        {
            ...
            {
                for (hdrtmp = mqueue->uselist; !hdrtmp->next->busy; hdrtmp=hdrtmp->next);
                hdr             = hdrtmp->next;
                hdrtmp->next    = hdr->next;
            }

            /* hdr points at where to read from (now located in the freelist BUT
             * marked as busy until data is copied ) */

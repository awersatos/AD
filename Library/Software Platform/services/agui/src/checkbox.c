...
void checkbox_draw(obj_t *obj)
{
    ...
    canvas = agui->canvas;
    checkcolor = obj->enabled ? agui->palette.edittext : agui->palette.disabled_dark;
    ...
    {
        color = agui->palette.disabled_light;
    }
    else
    {
    ...
    agui_draw_body_2d(agui, x1, y1, checkbox_size, checkbox_size, true, true, color, checkbox->relief);
    ...
    if ( checkbox->checked )
    {
        ...
        graphics_draw_arc( canvas, x1 - 5 * checkbox_size, y1 + 2 + (5 * checkbox_size) + (checkbox_size / 4), 7 * checkbox_size, 40, 3, checkcolor );
        graphics_draw_arc( canvas, x1 + 6 * checkbox_size, y1 + 1 + (5 * checkbox_size), 7 * checkbox_size, 138, 5, checkcolor );
#endif
    }
